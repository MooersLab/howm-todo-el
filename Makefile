# Makefile for the howm-todo Emacs Lisp package.
#
# Common targets:
#   make            - build everything (compile + info)
#   make compile    - byte-compile howm-todo.el
#   make test       - run the ERT suite in batch Emacs
#   make info       - generate howm-todo.info from howm-todo.texi
#   make html       - generate howm-todo.html from howm-todo.texi
...
SOURCES  = howm-todo.el
COMPILED = $(SOURCES:.el=.elc)
TESTS    = howm-todo-tests.el
TEXI     = howm-todo.texi
INFO     = howm-todo.info
HTML     = howm-todo.html
...
uninstall:
	...
	rm -f "$(DESTDIR)$(ELISPDIR)/howm-todo.el"
	rm -f "$(DESTDIR)$(ELISPDIR)/howm-todo.elc"
